 ...
 $user_check_query = "SELECT * FROM teacher WHERE username='$username'  LIMIT 1 ";
  ...
  if (count($errors) == 0) {
    ...
    $query = "INSERT INTO teacher  (username,password_1,firstname,lastname,salary,mobile,approved)
           VALUES('$username','$password','$firstname','$lastname','$salary',$mobile,'false')";
...
if (isset($_POST['login_teacher'])){
    ...
    echo $username;
    echo $password;
	  $q = mysqli_query($db, "SELECT * FROM teacher WHERE username='$username' and password_1='$password' and approved='true'");

    $res = mysqli_num_rows($q);

 		if($res == 1){
      $res = mysqli_query($db , "SELECT * FROM teacher  WHERE approved='true' " );
